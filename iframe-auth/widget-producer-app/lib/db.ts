...
// The CLI resolves file:./dev.db relative to schema.prisma (→ prisma/dev.db),
// but the runtime engine resolves it relative to process.cwd() (→ root).
// Passing the URL directly to the constructor avoids the env-var timing issue.
function getAbsoluteDatabaseUrl(): string {
  ...
  // Relative path: resolve against process.cwd() and return absolute
